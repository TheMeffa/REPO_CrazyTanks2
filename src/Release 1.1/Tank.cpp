...
void Tank::setTankConfiguration(const bool& isItHeroTank, GameMap &gameMapAll)
{
...
void Tank::setHeroLives(const int& heroLives)
{
	heroLives_ = heroLives;
}

void Tank::setIsAlive(const bool& isAlive)
{
	isAlive_ = isAlive;
}

void Tank::setPosition(const int & x, const int & y)
{
	positionX_ = x;
	positionY_ = y;
}
...
void Tank :: enemyAI(const int& heroPositionX, const int& heroPositionY) {
	if (isAlive_ == true) {

		if (way_ <= 0) {
			moveDirection_ = static_cast <MoveDirection> (rand() % COUNT_DIRECTIONS);
			way_ = rand() % MAX_ETANK_WAY + MIN_ETANK_WAY;
		}
...
void Tank::movement(GameMap& allGameMap) {
	...
	if(isAlive_ == true){
		...
		case UP:
			if (allGameMap.gameMap[positionY_ - 1][positionX_].getCurrentState() == EMPTY_BLOCK ||
				allGameMap.gameMap[positionY_ - 1][positionX_].getCurrentState() == PORTAL_POINT) {
		...
		case LEFT:
			if (allGameMap.gameMap[positionY_][positionX_ - 1].getCurrentState() == EMPTY_BLOCK ||
				allGameMap.gameMap[positionY_][positionX_ - 1].getCurrentState() == PORTAL_POINT) {
...
bool Tank::isEnoughClearSpace(GameMap& gameMapAll, const int& x, const int& y)
{
	...
	for (int i = y - MINIMAL_TANK_DISTANCE; i <= MINIMAL_TANK_DISTANCE * 2; i++) {
		for (int j = x - MINIMAL_TANK_DISTANCE; j <= MINIMAL_TANK_DISTANCE * 2; j++)
			if (gameMapAll.gameMap[i][j].getCurrentState() == ENEMY_TANK || 
				gameMapAll.gameMap[i][j].getCurrentState() == HERO_TANK) {
					return false;
				}
